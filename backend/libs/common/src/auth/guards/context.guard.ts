...
import { CLS_GUARD_OPTIONS, ClsGuard, ClsGuardOptions, ClsServiceManager } from 'nestjs-cls';
...
export class ContextGuard extends ClsGuard {
   ...
   */
  private async extractUserFromJwt(context: ExecutionContext): Promise<JwtData | undefined> {
    const contextType = context.getType();

    switch (contextType) {
      case 'http':
        const request = context.switchToHttp().getRequest();
        const jwt = request.cookies[env['JWT_NAME'] as string];
        if (!jwt) {
          return undefined;
        }
        try {
      ...
      case 'rpc':
        const metadata: Metadata = context.switchToRpc().getContext();
        const grpcJwt: string = metadata.get(env['JWT_NAME'] as string)[0] as string;
